    ...
    public class Config
    {
        ...
        public static IEnumerable<Client> GetClients()
        {
            return new List<Client>
            {
                ...
                {
                    //客户端的唯一ID
                    ClientId = "client_name_password",
                    // 客户端机密列表 - 访问令牌端点的凭据。
                    ClientSecrets =
                    {
                        new Secret("secret_name_password".Sha256())
                    },
                    //刷新令牌时，将刷新刷新令牌的生命周期（按SlidingRefreshTokenLifetime中指定的数量）。生命周期不会超AbsoluteRefreshTokenLifetime。
                    RefreshTokenExpiration = TokenExpiration.Sliding,
                    //指定此客户端是否可以请求刷新令牌（请求offline_access范围）
                    ...
                    RequireClientSecret = false,
                    //指定允许客户端使用的授权类型。使用GrantTypes该类进行常见组合。
                    AllowedGrantTypes = new List<string>{ "name_password" },
        ...
        public static IEnumerable<ApiResource> GetApis()
        {
            return new List<ApiResource>
            {
                ...
                new ApiResource("resource_api", "Resource API service")
            };
        }
